...
public class CardDisplayer : MonoBehaviour
{
    ...
    private async UniTaskVoid Display()
    {
        while (!destroyCancellationToken.IsCancellationRequested)
        {
            ...
            SetText(cardInformation);
            await UniTask.WaitForSeconds(0.25f)
            .AttachExternalCancellation(destroyCancellationToken);
        }
    }
    private void SetText(CardInformation cardInformation)
    {
        buff.text = "<color=green>" + (cardInformation.IsPercentageBased() ? " %" : " +") + cardInformation.cardBonus.GetBonusAmount(cardData.level);
        buff.text += " </color>" + cardInformation.cardBonus.cardBonusType.ToString();
        ...
        rarity.text = cardInformation.rarity.ToString();
    }
